...
def get_flops_pytorch_llava(trainer, input_res,
    ...
    add_flops_counting_methods(trainer.model) # counting 할 수 있도록 model 설정해주는 것
    #trainer.model = model
    trainer.model.train()
    trainer.model.start_flops_count(ost=ost, verbose=verbose, ignore_list=ignore_modules)
    ...
    batch = {}
    batch["input_ids"] = torch.arange(157 * 2).reshape([2,157]).long().cuda()
    batch["labels"] = torch.ones(2, 157).long().cuda()
    batch["attention_mask"] = torch.ones(2, 157).bool().cuda()
    batch["images"] = torch.ones(2, 5, 3, 336, 336).cuda()
    # _ = flops_model(batch)
    trainer.temp_forward(batch)

    #ctx_manager = torch.cpu.amp.autocast(cache_enabled=False, dtype=torch.bfloat16)
    #with ctx_manager:
    # outputs = model(**batch)


    forward_flops_count, backward_flops_count, params_count, fc_params_count, buffers_count = trainer.model.compute_average_flops_cost()
    ...
    trainer.model.stop_flops_count()
...
# ---- Internal functions
def batch_counter_hook(module, args, kwargs, output):
    input = kwargs["images"]
    batch_size = 1
    if len(input) > 0:
...
def add_backward_batch_counter_hook_function(module):
    if hasattr(module, '__batch_counter_handle__'):
        ...
        return
    # print("added", module)
    handle = module.register_backward_hook(batch_counter_hook, with_kwargs=True)
...
def add_batch_counter_hook_function(module):
    if hasattr(module, '__batch_counter_handle__'):
        ...
        return
    # print("added!!", module)
    handle = module.register_forward_hook(batch_counter_hook, with_kwargs=True)
    module.__batch_counter_handle__ = handle
