...
from utils.block_utils import get_llava_blockwise_flops, MODEL_BLOCK_DICT, MODEL_TRAINABLE_BLOCK_DICT
...
class LLaVATrainer(SFTTrainer):
    ...
    def set_flops_dict(self, flops_dict):
        forward_flops, backward_flops = get_llava_blockwise_flops(flops_dict)
    ...
    def delete_accelerator(self):
        """
        Deletes the current Accelerator instance by resetting its state.
        """
        if AcceleratorState().initialized:
            AcceleratorState()._reset_state()
            print("Accelerator state has been successfully reset.")
        else:
            print("No Accelerator state to reset.")
